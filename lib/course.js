  ...
  return new Promise(function(resolve, reject) {
    request(url, function (error, response, body) {
      if (!error) {
        ...
        var value = row.next().next().children().first().next().next(); // first value of first prop
          ...
          for (var j = 0; j < item.attr('colspan'); j++) {
            if (value.text().trim().length > 0) {
              newCategory.props.push({
                name: prop.text(),
                value: (prop.text() === 'Score') ? value.text() : parseFloat(value.text())
              });
            } else {
              newCategory.props.push({
                name: prop.text()
        ...
        var scores = [];
        var score = row.children().first().next().next();
        ...
        for (var i = 0; i < numCategories; i++) {
          ...
          for (var j = 0; j < category.props.length; j++) {
            newScore[category.props[j].name.toLowerCase()] = (category.props[j].name === 'Score') ? score.text() : parseFloat(score.text());
            score = score.next();
          }
        ...
        if (item.attr('colspan') == 2) {
          course.overallRank = parseFloat(row.children().last().prev().text());
